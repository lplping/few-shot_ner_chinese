...
class Trainer(object):
    def __init__(self, train_data=None, dev_data=None, test_data=None, model=None, process=None, args=None, logger=None, loss=None, metrics=None, device=None) -> None:
        self.train_data = train_data
        self.dev_data = dev_data
        self.test_data = test_data
        self.model = model
        self.process = process
        # print('self.process',self.process.mapping)
        self.logger = logger
    ...
    def train(self):
        # print('self.process.mapping',self.process)
        # print('self.process.mapping', self.process.mapping)
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    # print('batch',batch)
                    preds,raw_words,old_label =self._step(batch, mode="dev")
                    # print('preds',preds)
                    ...
                    # src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first, target_span, raw_words, old_label = batch
                ...
                print('f1', eva_result['f'],'recall', eva_result['rec'],'precision', eva_result['pre'])
                ...
                if eva_result['f'] >= self.best_metric:  # this epoch get best performance
                    ...
                    if self.save_path is not None:      # need to save model
                        #保存预测结果
                        assert  len(texts)==len(labels)==len(old_labels)
                        with open('pred.txt','w',encoding='utf-8') as f:
                            for line_txt,line_l,line_pre in zip(texts,old_labels,labels):
                                for txt,l,pre in zip(line_txt,line_l,line_pre):
                                    f.write(txt+'\t'+l+'\t'+pre+'\n')
                                f.write('\n')
    ...
    def _step(self, batch, mode="train"):
        if mode=="dev": # dev: compute metric
            # print('-------进入验证--------')
            src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first, target_span,raw_words,old_labels = batch
            ...
            return pred,raw_words,old_labels
        elif mode=="test":  # test: just get pred
            src_tokens, src_seq_len, first = batch
            pred = self.model.predict(src_tokens, src_seq_len, first)
            return pred
        else:   # train: get loss
            src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first, target_span,_,_ = batch  #传入的长度是实际序列长度
            # print('%%%%%%%%%%%%%%%%%')
            # print(src_seq_len, tgt_seq_len)#([ 20,  13, 279]) tensor([11,  8, 66])
            # print('tgt_tokens', tgt_tokens)
